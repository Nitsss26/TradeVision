...
import {
    ...
    ShoppingCart, X, ChevronRight, MessageSquare, Package
} from 'lucide-react';
        ...
        <header className="sticky top-0 z-50 bg-black border-b border-zinc-800 font-sans">
            ...
            <div className="flex items-center justify-between px-4 lg:px-8 py-4 gap-6">
                ...
                <form onSubmit={handleSearch} className="hidden md:flex flex-1 max-w-3xl relative z-10">
                    <div className="flex w-full bg-white rounded-full overflow-hidden shadow-[0_0_15px_rgba(37,99,235,0.1)] focus-within:shadow-[0_0_20px_rgba(37,99,235,0.25)] transition-all duration-300 ring-2 ring-transparent focus-within:ring-blue-500/20">
                        <div className="flex items-center px-4 border-r border-gray-100 cursor-pointer hover:bg-gray-50 transition-colors bg-white">
                            <span className="text-zinc-700 text-sm font-semibold">Products</span>
                            <ChevronDown className="w-3 h-3 text-zinc-400 ml-2" />
                        </div>
                        <input
                            type="text"
                            placeholder="What are you looking for..."
                            className="flex-1 px-4 py-3 bg-white text-black placeholder-zinc-400 outline-none text-[15px] font-medium"
                            value={searchQuery}
                            onChange={(e) => setSearchQuery(e.target.value)}
                        />
                        <button
                            type="submit"
                            className="px-8 py-2 bg-blue-600 hover:bg-blue-700 text-white font-bold transition-all duration-300 flex items-center gap-2 m-1 rounded-full text-[14px] shadow-lg shadow-blue-600/20 active:scale-95"
                        >
                    ...
                    <div className="flex items-center gap-3 ml-2 border-l border-zinc-800 pl-4">
                        {user ? (
                            ...
                            <div className="flex items-center gap-2">
                                <Link to="/login" className="text-sm font-semibold text-zinc-300 hover:text-white">Sign In</Link>
                                <Link to="/register" className="px-4 py-1.5 bg-blue-600 hover:bg-blue-700 text-white text-sm font-bold rounded-full transition-all shadow-lg shadow-blue-900/20">
                                    Join Free
                                </Link>
                            </div>
                        )}
            ...
            <div className="hidden md:flex items-center gap-8 px-4 lg:px-8 bg-zinc-950 border-t border-zinc-900 h-10 shadow-lg relative z-40">
                ...
                <div className="flex items-center gap-8">
                    ...
                    <Link to="/manufacturers" className="text-zinc-300 hover:text-white transition-colors text-[13px] font-medium tracking-wide hover:underline decoration-blue-500 decoration-2 underline-offset-4">Verified Manufacturers</Link>
                ...
                <div className="md:hidden bg-zinc-900 border-t border-zinc-800 p-4 absolute w-full left-0 top-full shadow-2xl z-40 animate-in slide-in-from-top-2">
                    ...
                    <div className="flex flex-col gap-1">
                        ...
                        <Link to="/manufacturers" className="text-zinc-300 py-3 border-b border-zinc-800 flex justify-between items-center hover:bg-zinc-800/50 px-2 rounded">
                            Verified Suppliers <ChevronRight className="w-4 h-4 text-zinc-500" />
                        </Link>
